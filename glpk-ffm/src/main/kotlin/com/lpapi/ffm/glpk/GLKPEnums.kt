package com.lpapi.ffm.glpk

import org.glpk.java.*

enum class GLPKObjective(val description: String, val value: Int) {
  MINIMIZE("MINIMIZE", GLPK.GLP_MIN()),
  MAXIMIZE("MAXIMIZE", GLPK.GLP_MAX());
}

enum class GLPKVarKind(val description: String, val value: Int) {
  CONTINUOUS("CONTINUOUS", GLPK.GLP_CV()),
  BOOLEAN("BOOLEAN", GLPK.GLP_BV()),
  INTEGER("INTEGER", GLPK.GLP_IV())
}

enum class GLPKBoundType(val description: String, val value: Int) {
...
enum class GLPKMipStatus(val description: String, val value: Int) {
...
enum class GLPKStatus(val description: String, val value: Int) {
  UNDEFINED("UNDEFINED", GLPK.GLP_UNDEF()),
  FEASIBLE("FEASIBLE", GLPK.GLP_FEAS()),
...
enum class GLPKMessageLevel(val description: String, val value: Int) {
...
enum class GLPKFeatureStatus(val description: String, val value: Int) {
  ON("ON", GLPK.GLP_ON()),
