...
class GurobiLpSolver(model: LPModel) : LPSolver<GRBModel>(model) {
  ...
  override fun solve(): LPSolutionStatus {
    try {
      ...
      val solutionStatus = getSolutionStatus(grbModel?.get(GRB.IntAttr.Status)
      )
    ...
    model.constraints.allValues().forEach { lpConstraint ->
      try {
        ...
        val modelConstraint : GRBConstr? =
            grbModel?.addConstr(generateExpression(lpConstraint.lhs),
                gurobiOperator,
  ...
  private fun generateExpression(expr: LPExpression): GRBLinExpr? {
    try {
      ...
      reducedExpr.expression.forEach { term ->
        if (term.isConstant()) {
          term.coefficient?.let {const ->  linExpr.addConstant(const) }
        } else {
          term.coefficient?.let {const -> linExpr.addTerm(const, variableMap[term.lpVarIdentifier]) }
        }
      }
      return linExpr
    } catch (e: Exception) {
