...
class GurobiLpSolverTest {
  ...
  private fun setParameter(solver: GurobiLpSolver, field: String, value: Any?) {
    solver.javaClass.getDeclaredField(field).let { it ->
      it.isAccessible = true
      it.set(solver, value)
    }
  }
  private fun setModel(solver: GurobiLpSolver, model: GRBModel?) =
      setParameter(solver, "grbModel", model)

  private fun setVariableMap(solver: GurobiLpSolver, variableMap: MutableMap<String, GRBVar>) =
      setParameter(solver, "variableMap", variableMap)

  private fun setConstraintMap(solver: GurobiLpSolver, variableMap: MutableMap<String, GRBConstr>) =
  ...
  fun testGrbVarTypeComputation() {
    ...
    LPVarType.values().forEach { lpVarType ->
      assertNotNull(model.getGurobiVarType(lpVarType), "Gurobi Model variable type not found for LP " +
          "type $lpVarType")
    }
  }
  ...
  fun testGrbOperatorTypeComputation() {
    ...
    LPOperator.values().forEach { lpOperator ->
      assertNotNull(model.getGurobiOperator(lpOperator), "Gurobi Model operator type not found for LP " +
          "Operator $lpOperator")
    }
  }
  ...
  fun testGrbObjectiveType() {
    ...
    LPObjectiveType.values().forEach { lpObjectiveType ->
      assertNotNull(model.getGurobiObjectiveType(lpObjectiveType), "Gurobi Model Objective Type node found " +
  ...
  private fun argsForInitObjectiveFunction() = Stream.of (
      Arguments.of(
          ...
          LPModel("test").apply{
            ...
            this.objective.expression.addTerm("a", "x").add("b")
          },
          mock<GRBModel>{
            on{ addVar(0.0, 1.0, 0.0, GRB.BINARY, "x")}.thenReturn(mockedVar)
          },
      ...
      Arguments.of(
          ...
          LPModel("test").apply{
            this.variables.add(LPVar("x", LPVarType.BOOLEAN))
            this.objective.expression.addTerm(2, "x").add(3)
          },
          mock<GRBModel>{
            on{ addVar(0.0, 1.0, 0.0, GRB.BINARY, "x")}.thenReturn(mockedVar)
      ...
      Arguments.of(
          ...
          LPModel("test").apply{
            ...
            this.objective.expression.addTerm(3, "x").add(2)
          },
          mock<GRBModel>{
            on{ addVar(0.0, 1.0, 0.0, GRB.BINARY, "x")}.thenReturn(mockedVar)
          },
    ...
    return Stream.of(
        Arguments.of(
            ...
            LPModel("test").apply {
              ...
              this.constraints.add(LPConstraint("test-constraint").apply { // generate constraint ax >= 3 - by
        ...
        Arguments.of(
            ...
            LPModel("test").apply {
              this.variables.add(LPVar("x", LPVarType.BOOLEAN))
              this.variables.add(LPVar("y", LPVarType.BOOLEAN))
              this.constraints.add(LPConstraint("test-constraint").apply { // generate constraint 2x >= 3 + 4y
        ...
        Arguments.of(
            ...
            LPModel("test").apply {
              this.variables.add(LPVar("x", LPVarType.BOOLEAN))
              this.variables.add(LPVar("y", LPVarType.BOOLEAN))
              this.constraints.add(LPConstraint("test-constraint").apply { // generate constraint 3y = 3 + 4x
        ...
        Arguments.of(
            ...
            LPModel("test").apply {
              this.variables.add(LPVar("x", LPVarType.BOOLEAN))
              this.variables.add(LPVar("y", LPVarType.BOOLEAN))
              this.constraints.add(LPConstraint("test-constraint").apply {
                // generate constraint 2 (x+y) + x >= 3 + 4x - 2y
                this.lhs.addTerm(2, "y").addTerm(2, "x").addTerm("x")
                this.operator = LPOperator.GREATER_EQUAL
                this.rhs.add(3).addTerm(4, "x").addTerm(-2, "y")
              })
            },
            mock<GRBModel>{
              on {addVar(0.0, 1.0, 0.0, GRB.BINARY, "x")}.thenReturn(mockedX)
  ...
  private fun argsForSolve() : Stream<Arguments> {
    ...
    val erroringMockedVar = mock<GRBVar>{
      on{get(GRB.DoubleAttr.X)}.thenThrow(GRBException("error"))
    }

    val lpModel =  LPModel("test").apply {
    ...
    return Stream.of(
        ...
        Arguments.of(
            ...
            null,
            LPSolutionStatus.ERROR,
            LPModelResult(solnStatus = LPSolutionStatus.ERROR),
            mutableMapOf<String, Number>()
        ),
        Arguments.of(
            ...
            LPModelResult(solnStatus = LPSolutionStatus.UNKNOWN),
            mutableMapOf<String, Number>()
        ),
        Arguments.of(
