...
class LPConstraintTest : LPParameterTest<LPConstraint>() {
  ...
  fun testEquality() {
    ...
    Assertions.assertFalse(c.equals(LPVar("constraint", LPVarType.BOOLEAN)),
        "Different param types  with same identifiers are not equal")

    assertEquals(c, LPConstraint("constraint", lhs, LPOperator.GREATER_EQUAL, rhs),
        "Default operation in the constructor is greater equal")
    assertNotEquals(c, LPConstraint("not-constraint", lhs, LPOperator.GREATER_EQUAL, rhs),
        "Constraints with different identifiers are not equal")
    assertNotEquals(c, LPConstraint("constraint", LPExpression(), LPOperator.GREATER_EQUAL, rhs),
        "Constraints with different LHS are not equal")
    assertNotEquals(c, LPConstraint("constraint", lhs, LPOperator.EQUAL, rhs),
        "Constraints with different operators are not equal")
    assertNotEquals(c, LPConstraint("constraint", LPExpression(), LPOperator.GREATER_EQUAL, LPExpression()),
        "Constraints with different RHS are not equal")
    c.operator= LPOperator.EQUAL
    assertEquals(c, LPConstraint("constraint", lhs, LPOperator.EQUAL, rhs),
