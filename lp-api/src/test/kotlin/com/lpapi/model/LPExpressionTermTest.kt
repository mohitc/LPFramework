...
class LPExpressionTermTest {
  ...
  fun testIsConstant() {
    assertTrue ( LPExpressionTerm(coefficient = 1.0, lpConstantIdentifier = null, lpVarIdentifier = null).isConstant(),
        "Term with only the coefficient should be constant")
    assertTrue ( LPExpressionTerm(coefficient = null, lpConstantIdentifier = "c", lpVarIdentifier = null).isConstant(),
        "Term with only the constant identifier should be constant")
    assertFalse ( LPExpressionTerm(coefficient = 1.0, lpConstantIdentifier = null, lpVarIdentifier = "x").isConstant(),
        "Term with variable defines should not be constant")
    assertFalse ( LPExpressionTerm(coefficient = null, lpConstantIdentifier = "c", lpVarIdentifier = "x").isConstant(),
        "Term with variable defines should not be constant")
  }
  ...
  fun testEquals() {
    ...
    assertNotEquals(termToCheck.hashCode(), differentTerm.hashCode(),
        "Different terms should not have the same hashCode")
    assertNotEquals(termToCheck.toString(), differentTerm.toString(),
