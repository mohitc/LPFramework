...
class LPModelResultTest {
  ...
  fun testEquality() {
    ...
    assertFalse(
      result, LPModelResult(LPSolutionStatus.TIME_LIMIT, 1.0, 100, 2.0),
      "Result with different solution status are not equal"
    )
    assertFalse(
      result, LPModelResult(LPSolutionStatus.OPTIMAL, 2.0, 100, 2.0),
      "Result with different objective are not equal"
    )
    assertFalse(
      result, LPModelResult(LPSolutionStatus.OPTIMAL, 1.0, 101, 2.0),
      "Result with different computation time are not equal"
    )
    assertFalse(
      result, LPModelResult(LPSolutionStatus.OPTIMAL, 1.0, 100, 2.1),
