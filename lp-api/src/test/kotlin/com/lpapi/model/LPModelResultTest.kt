...
class LPModelResultTest {
  ...
  fun testEquality() {
    ...
    val assertTrue = fun(a: LPModelResult, b: LPModelResult, assertion: String) {
    ...
    val assertFalse = fun(a: LPModelResult, b: LPModelResult, assertion: String) {
    ...
    Assertions.assertFalse(result.equals(LPConstant("x", 1)),
    ...
    Assertions.assertTrue(result.status == LPSolutionStatus.OPTIMAL,
        "Status is populated correctly" )
    Assertions.assertTrue(result.objective == 1.0,
        "Objective is populated correctly" )
    Assertions.assertTrue(result.mipGap == 2.0,
        "MIP Gap is populated correctly" )
    Assertions.assertTrue(result.computationTime == 100L,
        "Computation time is populated correctly" )

    assertTrue(result, LPModelResult(LPSolutionStatus.OPTIMAL, 1.0, 100, 2.0),
    "If all parameters are equal, the model results are equal")

    assertFalse(result, LPModelResult(LPSolutionStatus.TIME_LIMIT, 1.0, 100, 2.0),
        "Result with different solution status are not equal")
    assertFalse(result, LPModelResult(LPSolutionStatus.OPTIMAL, 2.0, 100, 2.0),
        "Result with different objective are not equal")
    assertFalse(result, LPModelResult(LPSolutionStatus.OPTIMAL, 1.0, 101, 2.0),
        "Result with different computation time are not equal")
    assertFalse(result, LPModelResult(LPSolutionStatus.OPTIMAL, 1.0, 100, 2.1),
