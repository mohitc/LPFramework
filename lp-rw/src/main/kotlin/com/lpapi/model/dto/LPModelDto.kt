...
data class LPConstraintDto(val identifier: String, val lhs: LPExpressionDto, val operator: LPOperator, val rhs: LPExpressionDto)

data class LPModelDto(val identifier: String, val objectiveType: LPObjectiveType, val objective: LPExpressionDto,
                      val constants: Map<String, List<LPConstantDto>>, val vars: Map<String, List<LPVarDto>>,
                      val constraints: Map<String, List<LPConstraintDto>>)

data class LPVarResultDto(val identifier: String, val result: Number)

data class LPModelResultDto(val computed: Boolean, val status: LPSolutionStatus? = null, val objective: Double? = null, val computationTime: Long? = null,
                            val mipGap: Double? = null, val vars: List<LPVarResultDto>? = null)