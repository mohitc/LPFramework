...
import io.github.mohitc.lpapi.model.dto.LPModelDto
...
class LPModelParser(
  format: LPModelFormat = LPModelFormat.YAML,
) {
  ...
  fun generateModelDto(model: LPModel): LPModelDto {
            ...
            entry.key to
              entry.value
                ...
                .map {
                  LPConstraintDto(
                    it.identifier,
                    generateExpressionDto(it.lhs),
                    it.operator,
                    generateExpressionDto(it.rhs),
                  )
                }
          )
        }.toMap()
    ...
    return LPModelDto(
      ...
      generateExpressionDto(model.objective.expression),
      constantMap,
      varMap,
      constraintMap,
    )
      ...
      mapEntry.value.forEach { constraintDto ->
        model.constraints.add(
          ...
          LPConstraint(
            ...
            generateExpression(constraintDto.lhs),
            constraintDto.operator,
            generateExpression(constraintDto.rhs),
          ),
        )
      }
    }
      ...
      if (solution.objective != null) {
        LPModelResultDto(
          ...
          model.variables
            .allValues()
            .toList()
            .map { lpVar -> LPVarResultDto(lpVar.identifier, lpVar.result) },
        )
      } else {
   ...
   */
  private fun generateExpressionDto(lpExpression: io.github.mohitc.lpapi.model.LPExpression): LPExpressionDto =
    LPExpressionDto(
   ...
   */
  private fun generateExpression(lpExpressionDto: LPExpressionDto): io.github.mohitc.lpapi.model.LPExpression {
    val lpExpression =
