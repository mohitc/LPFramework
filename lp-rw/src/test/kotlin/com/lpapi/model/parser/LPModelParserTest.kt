...
class LPModelParserTest {
  ...
  private fun modelsForTest() = Stream.of(
      Arguments.of("Sample model to JSON",
          sampleModel(false),
          "testModel.json",
          LPModelParser(LPModelFormat.JSON)),
      Arguments.of("Sample model to YAML",
          sampleModel(false),
          "testModel.yaml",
  ...
  fun testReadWriteOfPrimitiveModel(testCase: String, model: LPModel, fileName: String, lpModelParser: LPModelParser) {
    ...
    Assertions.assertEquals(newModel!!.identifier, model.identifier,
        "Identifiers for the model were re-parsed correctly")
    Assertions.assertEquals(newModel.variables, model.variables,
        "Variables for the model were re-parsed correctly")
    Assertions.assertEquals(newModel.constants, model.constants,
        "Constants for the model were re-parsed correctly")
    Assertions.assertEquals(newModel.constraints, model.constraints,
        "Constraints for the model were re-parsed correctly")
    Assertions.assertEquals(newModel.objective, model.objective,
        "Objective fn for the model were re-parsed correctly")
    Assertions.assertEquals(newModel, model,
  ...
  fun testReadWriteOfPrimitiveModelWithResult(testCase: String,
                                              model: LPModel,
                                              fileName: String,
    ...
    log.info { "Reading model from file in JSON format" }
    val newModel = lpModelParser.readFromFile(fileName)
    Assertions.assertNotEquals(newModel, model,
  ...
  private fun resultDtosForTest() = Stream.of(
      Arguments.of("DTO with computed false is parsed without errors",
          LPModelParser(LPModelFormat.JSON),
          sampleModel(false),
          LPModelResultDto(computed = false), true),
      Arguments.of("DTO with null status results in false",
          LPModelParser(LPModelFormat.JSON),
          sampleModel(false),
          LPModelResultDto(computed = true, status = null), false),
      Arguments.of("DTO with objective but no variables results in false",
          LPModelParser(LPModelFormat.JSON),
          sampleModel(false),
          LPModelResultDto(computed = true, status = LPSolutionStatus.OPTIMAL, objective = 1.0), false),
      Arguments.of("DTO with bad variable identifier results in false",
          LPModelParser(LPModelFormat.JSON),
          sampleModel(false),
          ...
          false),
      Arguments.of("DTO with correct variables results in true",
          LPModelParser(LPModelFormat.JSON),
          sampleModel(false),
          LPModelResultDto(computed = true, status = LPSolutionStatus.OPTIMAL, objective = 0.0,
              vars = listOf(
                  LPVarResultDto("X", 1),
                  LPVarResultDto("Y", 1),
                  LPVarResultDto("Z", 0),
  ...
  fun testPopulateModelResult(testCase: String,
