...
class SCIPProblem() {
  ...
  companion object {
    fun getPtrFromPtrPtr(varPtrPtr: MemorySegment): MemorySegment =
        varPtrPtr.get(SCIP.C_POINTER, 0)

    fun getPtrPtrFromPtr(arena: Arena, varPtr: MemorySegment): MemorySegment {
      val varPtrPtr = arena.allocateFrom(SCIP.C_POINTER, varPtr)
      return varPtrPtr
    }

    fun createPtrArray(it: Arena, vars: List<Variable>): MemorySegment {
  ...
  fun includeDefaultPlugins() =
      SCIPRetCode.fromVal(SCIPPlugins.SCIPincludeDefaultPlugins(scipPtr))

  fun createVar(varName: String, lb: Double, ub: Double, objective: Double, varType: SCIPVarType): Variable? {
    Arena.ofConfined().use {
      val varPtrPtr: MemorySegment = scipArena.allocate(SCIP.C_POINTER) // VAR**
      var retCode = SCIPRetCode.fromVal(SCIP.SCIPcreateVarBasic(scipPtr, varPtrPtr, it.allocateFrom(varName), lb, ub, objective, varType.value))
  ...
  fun createConstraint(name: String, vars: List<Variable>, coefficients: List<Double>, lb: Double, ub: Double): Constraint? {
    Arena.ofConfined().use {
      val constraintPtrPtr = scipArena.allocate(SCIP.C_POINTER)
      var retCode = SCIPRetCode.fromVal(SCIPConstraint.SCIPcreateConsBasicLinear(
          scipPtr,
          constraintPtrPtr,
  ...
  fun getSolVal(sol: Solution, variable: Variable) = SCIP.SCIPgetSolVal(scipPtr, sol.ptr, variable.ptr)

  fun setVariableObjective(variable: Variable, objective: Double) =
  ...
  fun writeOriginalProblem(fileName: String, extension: String, genericNames: SCIPBool) =
      Arena.ofConfined().use {
        SCIP.SCIPwriteOrigProblem(scipPtr, it.allocateFrom(fileName), it.allocateFrom(extension), genericNames.value)
      }
}

open class CPointer(val ptr: MemorySegment)

class Variable(ptr: MemorySegment) : CPointer(ptr)

class Constraint(ptr: MemorySegment) : CPointer(ptr)

class Solution(ptr: MemorySegment) : CPointer(ptr)
